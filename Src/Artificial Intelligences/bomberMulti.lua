...
nearestEnemyPosition = getNearestEnemyPosition(AI)
...
function computeDistanceFromPlayer()
    ...
    if aiPosition["x"] ~= nearestEnemyPosition["x"] and aiPosition["y"] ~= nearestEnemyPosition["y"] then
       return -1
    elseif aiPosition["x"] ~= nearestEnemyPosition["x"] and aiPosition["y"] ~= nearestEnemyPosition["y"] then
        return 0
    end
    if aiPosition["x"] == nearestEnemyPosition["x"] then
        if nearestEnemyPosition["y"] > aiPosition["y"] then 
            increment = 1
        else
            increment = -1
        end
        for i = aiPosition["y"] + increment, nearestEnemyPosition["y"], increment do
            distance = distance + 1
            if map[aiPosition["x"]][i] == BLOCK or map[aiPosition["x"]][i] == BOX then
                return -1
            end
        end
    elseif aiPosition["y"] == nearestEnemyPosition["y"] then
        if nearestEnemyPosition["x"] > aiPosition["x"] then 
            increment = 1
        else
            increment = -1
        end
        for i = aiPosition["x"] + increment, nearestEnemyPosition["x"], increment do
            distance = distance + 1
